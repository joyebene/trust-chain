...
export default function Register() {
  ...
  return <main className="auth-shell"><div className="auth-brand"><Link href="/" className="brand"><Image src="/logo.jpeg" alt="Trust Chain" width={60} height={60} className="brand-logo" /><span>TRUST <b>CHAIN</b></span></Link></div><div className="auth-card wide"><div className="auth-title"><div className="gold-circle"><User /></div><h1>Create your account</h1><p>Join Trust Chain and manage your investment journey.</p></div>{error && <div className="alert error">{error}</div>}
    <form onSubmit={submit} className="form">
      ...
      <label>Full name<div className="input-wrap"><User /><input required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Your full name" /></div></label>

      <label>Email address<div className="input-wrap"><Mail /><input required type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="you@example.com" /></div></label><label>Phone number<div className="input-wrap"><User /><input required value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} placeholder="+234..." /></div></label><label>Password<div className="input-wrap"><LockKeyhole /><input required minLength={8} type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder="At least 8 characters" /></div></label><button disabled={loading} className="btn btn-gold btn-block">{loading ? <Loader2 className="spin" /> : "Create account"}</button></form>
